...
function buildSystemPrompt({ language = "en", context = {} } = {}) {
    ...
    const isPartnerLead = ["vendorEnquiry", "provider", "professionalProvider", "pendingServiceProvider", "pendingProfessionalProvider"].includes(sourceContext);
    const isPendingServiceProviderLead = sourceContext === "pendingServiceProvider";
    const isPendingProfessionalProviderLead = sourceContext === "pendingProfessionalProvider";
    ...
    const sourceHint =
        sourceContext === "enquiry"
            ...
            : isPendingServiceProviderLead
              ? `\n- Lead source: pending service provider onboarding (${sourceContext}). This vendor applied in the New Providers Request section${sourceCategory ? ` for category "${sourceCategory}"` : ""}${sourceRequirement ? ` with details "${sourceRequirement}"` : ""}${sourceRemarks ? `. Internal note: "${sourceRemarks}"` : ""}. This section is ONLY for home appliance partner onboarding. Follow this flow naturally: introduce yourself from Doorstep Hub, ask whether they are a vendor or home appliance technician, ask if they want to onboard and grow their business, explain that Doorstep Hub helps partners get home appliance service bookings using the live catalog summary in context (home appliances only), invite them to join the partner network, send partner onboarding WhatsApp, and close politely. Never mention unrelated services outside home appliances unless they are in the live home-appliance catalog.`
              : isPendingProfessionalProviderLead
                ? `\n- Lead source: pending professional provider onboarding (${sourceContext}). This partner applied in the New Professional Requests section${sourceCategory ? ` for category "${sourceCategory}"` : ""}${sourceRequirement ? ` with details "${sourceRequirement}"` : ""}${sourceRemarks ? `. Internal note: "${sourceRemarks}"` : ""}. Detect the professional vertical from the saved service/category and follow the matching pitch: religious services -> poojas, homas, pooja rituals; spa/salon -> women's pedicure, men's pedicure, pet grooming; PG/hostel -> nearby staying and accommodation. Ask clearly whether they provide those services. If they say yes, tell them to download the Doorstep Hub Partner app and start getting bookings now, then send partner onboarding WhatsApp and close politely. Use live professional catalog summary when available.`
                : isPartnerLead
...
- For pending service provider leads (pendingServiceProvider), home appliance services only. Use live catalog data filtered to home appliances.
- For pending professional provider leads (pendingProfessionalProvider), use vertical-specific examples: religious -> poojas/homas/pooja rituals; spa/salon -> women's pedicure, men's pedicure, pet grooming; PG/hostel -> nearby staying. Ask if they provide those services. If yes, tell them to download the Partner app and get bookings now.
