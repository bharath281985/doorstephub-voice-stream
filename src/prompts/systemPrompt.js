...
function buildSystemPrompt({ language = "en", context = {} } = {}) {
    ...
    const isPartnerLead = ["vendorEnquiry", "provider", "professionalProvider", "pendingServiceProvider"].includes(sourceContext);
    ...
    const sourceHint =
        sourceContext === "enquiry"
            ...
            : isPendingServiceProviderLead
              ? `\n- Lead source: pending service provider onboarding (${sourceContext}). This vendor applied in the New Providers Request section${sourceCategory ? ` for category "${sourceCategory}"` : ""}${sourceRequirement ? ` with details "${sourceRequirement}"` : ""}${sourceRemarks ? `. Internal note: "${sourceRemarks}"` : ""}. This is ONLY a vendor onboarding pitch call. Follow this flow naturally: introduce yourself from Doorstep Hub, ask whether they are a vendor or service technician, ask if they want to onboard and grow their business with Doorstep Hub, briefly explain that Doorstep Hub helps partners get more customer bookings and increase income, explain services in one short helpful line using the live catalog summary in context or get_live_catalog_snapshot (mention real categories/subcategories/services from the backend, never random examples), invite them to join the Doorstep Hub partner network, send the partner onboarding WhatsApp using send_whatsapp_message, and close politely. Never ask whether they personally need appliance service, home service, booking support, or customer help.`
            : isPartnerLead
...
- For pending service provider leads (pendingServiceProvider), use this vendor onboarding pitch only for the New Providers Request section: introduction -> ask if they are a vendor -> ask if they want onboarding to grow business -> explain Doorstep Hub partner benefits and real live-catalog services briefly -> invite them to join -> send partner WhatsApp -> close politely.
- For pending service provider leads, when explaining services, use the live catalog summary or get_live_catalog_snapshot. Mention real categories, subcategories, and services from the backend. Never use random hardcoded service examples.
